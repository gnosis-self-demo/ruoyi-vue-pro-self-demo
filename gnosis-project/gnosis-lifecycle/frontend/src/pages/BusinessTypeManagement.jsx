...
import {
  ...
  batchEnableBusinessTypes,
  batchDisableBusinessTypes,
  importBusinessTypes,
  exportBusinessTypes
...
const BusinessTypeManagement = () => {
  ...
  const loadData = async (params = {}) => {
    ...
    try {
      ...
      const response = await getBusinessTypeList(queryParams)
  ...
  const handleEdit = async (record) => {
    try {
      const response = await getBusinessTypeDetail(record.id)
  ...
  const handleBatchDelete = async () => {
    ...
    try {
      await batchDeleteBusinessTypes(selectedRowKeys)
  ...
  const handleBatchEnable = async () => {
    ...
    try {
      await batchEnableBusinessTypes(selectedRowKeys)
  ...
  const handleBatchDisable = async () => {
    ...
    try {
      await batchDisableBusinessTypes(selectedRowKeys)
  ...
  const handleImport = async (file) => {
    try {
      await importBusinessTypes(file)
  ...
  const handleExport = async () => {
    try {
      const searchValues = searchForm.getFieldsValue()
      const blob = await exportBusinessTypes(searchValues)
      const url = window.URL.createObjectURL(blob)
      const link = document.createElement('a')
      link.href = url
      link.download = `业务类型_${Date.now()}.xlsx`
      link.click()
  ...
  return (
    <div className="page-container">
      ...
      <Form form={searchForm} className="search-form" layout="inline">
        <Row gutter={16}>
          <Col span={6}>
            <Form.Item label="业务类型编码" name="code">
              <Input placeholder="请输入" allowClear />
            </Form.Item>
          </Col>
          <Col span={6}>
            <Form.Item label="业务类型名称" name="name">
              <Input placeholder="请输入" allowClear />
            </Form.Item>
          </Col>
          <Col span={6}>
        ...
        <Row gutter={16}>
          ...
          <Col span={6}>
            <Form.Item label="更新人 ID" name="updateUserId">
              <Input placeholder="请输入" allowClear />
            </Form.Item>
          </Col>
          <Col span={6}>
      ...
      {/* 新建/编辑弹窗 */}
      <Modal
        title={editingRecord ? '编辑业务类型' : '新建业务类型'}
        open={modalVisible}
        onOk={handleModalOk}
