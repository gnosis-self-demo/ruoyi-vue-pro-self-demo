...
import { templateApi } from '../api/signatureApi';
...
const templateTypeMap = {
  CONTRACT: { color: 'blue', text: '合同模板' },
  AGREEMENT: { color: 'green', text: '协议模板' },
  CERTIFICATE: { color: 'orange', text: '证明模板' },
  NOTICE: { color: 'purple', text: '通知模板' },
  OTHER: { color: 'default', text: '其他模板' },
};
...
const TemplateManage = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (modalType === 'create') {
        ...
        await templateApi.update({ id: currentRecord.id, ...values });
  ...
  const columns = [
    ...
    { title: '模板名称', dataIndex: 'templateName', key: 'templateName', width: 180, ellipsis: true },
    {
      ...
      width: 120,
      render: (val) => {
        const item = templateTypeMap[val] || { color: 'default', text: val };
        return <Tag color={item.color}>{item.text}</Tag>;
      },
    },
    { title: '模板内容', dataIndex: 'templateContent', key: 'templateContent', ellipsis: true },
    { title: '签章位置', dataIndex: 'signPosition', key: 'signPosition', width: 120, ellipsis: true },
      ...
      <Row gutter={[16, 16]} style={{ width: '100%' }}>
        <Col xs={24} sm={12} md={8} lg={6}>
          <Form.Item name="templateCode" label="模板编码">
            <Input placeholder="请输入模板编码" allowClear />
          </Form.Item>
        </Col>
        <Col xs={24} sm={12} md={8} lg={6}>
        ...
        <Col xs={24} sm={12} md={8} lg={6}>
          <Form.Item name="templateType" label="模板类型">
            <Select placeholder="请选择模板类型" allowClear>
              {Object.entries(templateTypeMap).map(([key, val]) => (
        ...
        <Col xs={24} sm={12} md={8} lg={6}>
          <Form.Item name="templateContent" label="模板内容">
            <Input placeholder="请输入模板内容" allowClear />
          </Form.Item>
        </Col>
        <Col xs={24} sm={12} md={8} lg={6}>
          <Form.Item name="signPosition" label="签章位置">
            <Input placeholder="请输入签章位置" allowClear />
          </Form.Item>
        </Col>
        <Col xs={24} sm={12} md={8} lg={6}>
  ...
  const renderModalForm = (formInstance) => (
    <Form form={formInstance} labelCol={{ span: 6 }} wrapperCol={{ span: 16 }}>
      ...
      >
        <Input placeholder="请输入模板编码" disabled={modalType === 'detail'} />
      </Form.Item>
      <Form.Item
        name="templateName"
        label="模板名称"
      ...
      >
        <Input placeholder="请输入模板名称" disabled={modalType === 'detail'} />
      </Form.Item>
      <Form.Item
        name="templateType"
        label="模板类型"
      ...
      >
        <Select placeholder="请选择模板类型" disabled={modalType === 'detail'}>
          {Object.entries(templateTypeMap).map(([key, val]) => (
            <Option key={key} value={key}>
              <Tag color={val.color}>{val.text}</Tag>
            </Option>
      ...
      <Form.Item name="templateContent" label="模板内容">
        <TextArea rows={4} placeholder="请输入模板内容" disabled={modalType === 'detail'} />
      </Form.Item>
      <Form.Item name="signPosition" label="签章位置">
        <Input placeholder="请输入签章位置" disabled={modalType === 'detail'} />
      </Form.Item>
      <Form.Item name="description" label="描述">
  ...
  return (
    <div>
      ...
      {/* 新建 / 编辑弹窗 */}
      <Modal
        title={{ create: '新建模板', edit: '编辑模板' }[modalType] || '模板'}
