...
COMMENT ON COLUMN sys_distributed_queue.message_body IS '消息体内容';
COMMENT ON COLUMN sys_distributed_queue.status IS '消息状态（pending/processing/failed/success）';
COMMENT ON COLUMN sys_distributed_queue.consumer_id IS '消费者标识';
COMMENT ON COLUMN sys_distributed_queue.attempt_count IS '重试次数';
...
-- 添加表注释
COMMENT ON TABLE sys_queue_result IS '队列结果表（用于同步等待）';
COMMENT ON COLUMN sys_queue_result.request_id IS '请求ID';
COMMENT ON COLUMN sys_queue_result.success IS '执行是否成功';
...
-- 添加表注释
COMMENT ON TABLE sys_distributed_lock IS '分布式锁表（用于协调多节点定时任务）';
